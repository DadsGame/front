...
const GameDetail = ({ game, latestsPosts, cookies }) => {
  ...
  async function addToWishList(id) {
    ...
    const wishUrl = new URL(
      `/games/addToWishlist/${game.id}`,
      process.env.NEXT_PUBLIC_MAIN_API_URL
    );
          ...
          <div className={styles[`details-container-${breakPointName}`]}>
            ...
            <div className={styles['game-detail-score']}>
              {game.aggregated_rating != null ? (
                <span>
                  score: {formatter.format(game.aggregated_rating / 100)}
                </span>
              ) : (
            ...
            <div className={styles['game-detail-add-lib']}>
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/add_game?game_name=${game.name.replace(
                      '&',
                      '%26'
      ...
      <GenericCard>
        <div className={styles['forum-header']}>
          ...
          <div className={styles['forum-header-btns']}>
            <Button
              variant="contained"
              onClick={() => router.push(`/posts/add?gid=${game.id}`)}
            >
              Add a post
            </Button>
            {latestsPosts != null && latestsPosts.length > 0 ? (
              <Button
                variant="contained"
                onClick={() => router.push(`/posts?gid=${game.id}`)}
              >
                See all posts
              </Button>
            ) : (
