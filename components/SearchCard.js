...
const SearchCard = ({game, cookies}) => {
    ...
    const skeleton = game == null || game.name == null || game.description == null ||game.image == null || game.id == null;

    if (skeleton) {
        return (
        <Card sx={{maxWidth: 345}}>
                <CardMedia sx={{bgColor: 'black'}}>
                <Skeleton variant="rectangular" animation="wave" width="100%" height={118}  sx={{bgcolor: 'grey.700'}} />
                </CardMedia>
                <CardContent>
    ...
    return (
        <Card sx={{maxWidth: 345, display: 'grid', gridTemplateRows: 'min-content max-content 1fr', gridTemplateColumns: '1fr'}}>
            ...
            <CardActions sx={{alignSelf: 'end'}}>
                {token !== ''
                    ? (
                        <Link size="small" color="primary" href={`/add_game?game_name=${game.name.replace('&', '%26')}&fromSearch=true&gameId=${game.id}`}>
                            Add to your library
                        </Link>
                    )
                    : ''
