    ...
    return(
        ...
        <div className={styles.cards}>
            ...
            <GenericCard>
                ...
                <span  style={{color: 'red', fontSize: 'large'}}>{currencyFormatter.format(stat.total_spent_players)}</span>
            </GenericCard>
            <GenericCard>
                <h2>Total of money gained by players</h2>
                <span  style={{color: 'green', fontSize: 'large'}}>{currencyFormatter.format(stat.total_revenue_players)}</span>
            </GenericCard>
            <GenericCard>
                <h2>User with the biggest library</h2>
                <div className={styles.winner}>
                <EmojiEventsIcon />
                <span  style={{color: 'gold', fontSize: 'large'}}>{stat.user_with_max_games}</span>
                </div>
            </GenericCard>
            <GenericCard>
...
export async function getServerSideProps() {
    const url = new URL ('/games/stats/global', process.env.NEXT_PUBLIC_MAIN_API_URL);
