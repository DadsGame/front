...
const checkIfGameTopicExists = async (gameTopicId) => {
  const gameTopicUrl = new URL(
    `/gametopics/${gameTopicId}`,
    ...
    const redirectIfNeeded = async () => {
      if (token == null || token === '') {
        return await router.push(
          '/login?alert=You must be logged in to add a post.'
        );
      } else if (
        router.query?.gid == null &&
        !router.asPath?.split('?')?.[1]?.startsWith('gid=')
      ) {
        setOpen(true);
      }
    };
