...
function Library({cookies}) {
    ...
    async function fetchLibrary() {
        ...
        response = await Promise.all(response.map(async (response) => {
            if(response.igdb_id == null) return response;
            const postsUrl =  new URL (`/posts/byGameTopic/filtered/${response.igdb_id}`, process.env.NEXT_PUBLIC_MAIN_API_URL);
