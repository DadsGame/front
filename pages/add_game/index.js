...
const checkGameShouldBeLocal = async (gameName, gameId) => {
  const gameMatchUrl = new URL(
    '/api/igdb/games/shouldBeLocal',
...
const checkIfGameIsInLibrary = async (token, gameId) => {
  const postGameUrl = new URL(
    `/games/userLibrary/isPresent/${gameId}`,
    process.env.NEXT_PUBLIC_MAIN_API_URL
  );
...
const postGamelocalToLibrary = async (formData, token) => {
  ...
  const alreadyExists = await checkIfGameAlreadyExistsLocal(
    formData.name,
    token
  );
  ...
  if (!alreadyExistBool) {
    ...
    const isGameInLibrary = await checkIfGameIsInLibrary(
      token,
      alreadyExists.id
    );
    if (isGameInLibrary[0].count > 0)
      return { error: 'game is already in library' };
  }
  const res = await fetch(postGameUrl.toString(), {
    headers: {
      'Content-Type': 'application/json',
    ...
    const redirectIfNeeded = async () => {
      if (token == null || token === '') {
        return await router.push(
          '/login?alert=You must be logged in to add a game.'
        );
      }
    };
    redirectIfNeeded();
