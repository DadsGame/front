    ...
    <div>
      <div>
        ...
        {games2 == null || games2.length === 0 ? (
          <span className={styles['not-enough-data']}>
            Not enough data to show this category.
          </span>
        ) : (
...
export async function getServerSideProps() {
  const res = await fetch(
    new URL('api/igdb/topten', process.env.NEXT_PUBLIC_BTB_API_URL)
  );
  ...
  const res4 = await fetch(
    new URL(
      '/games/averagePlaytime',
      process.env.NEXT_PUBLIC_MAIN_API_URL
    ).toString()
  );
  let games4 = await res4.json();
  games4 = await Promise.all(
    games4.map(async (g) => {
      if (g.igdb_id != null) {
        const url = new URL(
          '/api/igdb/games/byId',
